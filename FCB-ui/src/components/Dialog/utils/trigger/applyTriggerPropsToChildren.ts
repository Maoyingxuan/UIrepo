 ...
 * a FcbTriggerComponent or React Fragment (the same element returned by {@link getTriggerChild}).
 */
function cloneTriggerTree<TriggerChildProps>(
  ...
  if (isFishTrigger(child)) {
    /* eslint-disable @typescript-eslint/no-explicit-any */
    const grandchild = cloneTriggerTree(
    ...
    return React.cloneElement(
      child,
      triggerProps as TriggerChildProps & React.Attributes
    );
  }
}
